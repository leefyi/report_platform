...
class ReportDetail(models.Model):
    ...
    screenshots_path = models.ImageField(
        "测试图片，传入图片路径即可", upload_to='upload', default=None)
    ...
    report_create_time = models.DateTimeField(
        "创建时间", auto_now=False, auto_now_add=True)
    report_update_time = models.DateTimeField(
        "更新时间", auto_now=True, auto_now_add=False)
    platform_name = models.TextField("系统类型", max_length=16, default="android")
    case_step_time = models.TextField(
        "测试步长，json格式", max_length=2048, default=None)
...
class ReportInfo(models.Model):
    ...
    case_fail_sum = models.IntegerField(
        "失败case总和", default=0)
    case_skip_sum = models.IntegerField("跳过case综合", default=0, blank=True)
    ...
    report_uuid = models.CharField("每次运行唯一md5标识", max_length=256, default=None)
    report_info_create_time = models.DateTimeField(
        "创建时间", auto_now=False, auto_now_add=True)
    report_info_update_time = models.DateTimeField(
        "更新时间", auto_now=True, auto_now_add=False)
